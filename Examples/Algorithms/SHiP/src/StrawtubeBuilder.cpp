...
StrawtubeBuilder::StrawtubeBuilder(const Config& cfg)
 ...
 Acts::GeometryContext context = Acts::GeometryContext::dangerouslyDefaultConstruct();
 // Sort straws
 std::vector<StrawNode> allStraws;
 findStraws(gGeoManager->GetTopVolume(), Acts::Transform3::Identity(), allStraws);
 std::sort(allStraws.begin(), allStraws.end(), [&context](const auto& a, const auto& b) { return a.x < b.x; });
 ...
 for (const auto& straw : allStraws) {
    ...
    for (auto& cluster : clusters) {
        ...
        for (auto& s : cluster) {
            ...
            std::shared_ptr<const Acts::ISurfaceMaterial> materialParam = surfaceMaterial;

            // 2. Explicitly copy your bounds pointer so it matches the value-argument signature
            std::shared_ptr<const Acts::LineBounds> boundsParam = bounds;

            // 3. Construct using the precise value parameters
            auto detElement = std::make_shared<SHiPDetectorElement>(
                std::make_shared<const Acts::Transform3>(s.transform),
                boundsParam,
                2.0,
                materialParam
            );
        ...
        std::sort(surfaces.begin(), surfaces.end(), [&context](const auto& a, const auto& b) {
            return a->center(context).y() > b->center(context).y();
        });
