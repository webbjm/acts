...
SeedToParams::SeedToParams(Config config, Acts::Logging::Level level)
    : IAlgorithm("SeedToParams",Acts::getDefaultLogger("SeedToParams", level)), m_cfg(std::move(config)) {
...
ProcessCode SeedToParams::execute(const AlgorithmContext& ctx) const {
  ...
  for (const auto& indices : protoTracks) {
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      Acts::Vector3 uAxis = surf->localToGlobalTransform(ctx.geoContext).linear().col(0);
