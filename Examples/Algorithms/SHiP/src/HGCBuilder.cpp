...
HGCBuilder::HGCBuilder(const Config& cfg)
 ...
 std::sort(m_layers.begin(), m_layers.end(), [&context](const auto& a, const auto& b) {
     return a->surfaceRepresentation().center(context).x() < 
            b->surfaceRepresentation().center(context).x();
 });
}
