...
class SHiPDetectorElement : public Acts::SurfacePlacementBase {
 public:
  ...
  double thickness() const { return m_elementThickness; }

  /// Return local to global transform associated with this context
  /// NOTE: Replaced deprecated 'transform' override with 'localToGlobalTransform'
  const Acts::Transform3& localToGlobalTransform(
      const Acts::GeometryContext& gctx) const final;

    /// Return the nominal local to global transform (Removed 'override')
  ...
  /// MANDATORY CONSTRUCT FIX: Satisfies the pure virtual constraint of the base class
