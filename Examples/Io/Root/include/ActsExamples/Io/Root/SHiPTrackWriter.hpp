...
///
/// Safe to use from multiple writer threads - uses a std::mutex lock.
class SHiPTrackWriter final : public WriterT<ConstTrackContainer> {
 public:
  struct Config {
    ...
    /// Output filename.
    std::string filePath = "recoTracks.root";
    /// Name of the output tree.
    std::string treeName = "recoTracks";
    /// File access mode.
  ...
  SHiPTrackWriter(const Config& config, Acts::Logging::Level level);
  ~SHiPTrackWriter() override;
